...
'''
Tests the './config/config_ingest_targets.py' module.

This test file is in FilePusher/tests
and config_ingest_targets.py is in project root ('.').
...
import unittest
import yaml
import config.config_ingest_targets as cis
...
class TestYamlIngestConfig(unittest.TestCase):
    ...
    def test_conf_is_machine(self):
        ...
        with open(cis.FULL_PATH, 'r') as conf:
            y = yaml.load(conf)
            self.assertEqual(y['Targets'][0]['IsMachine'], False)
