...
"""
Tests the './config_service.py' module.

This test file is in FilePusher/tests
and config_service.py is in project root ('.').
...
from .. configuration import config_service as cis


class TestYamlIngestService(unittest.TestCase):
    """ This is the test for the service marshaller. """
    ...
    def test_conf_db_address(self):
        """ Check DB Address. """
        with open(cis.FULL_PATH, 'r') as conf:
            service_config = yaml.load(conf)
            self.assertEqual(service_config['Database']['DBAddr'], '192.168.208.61:5432')
